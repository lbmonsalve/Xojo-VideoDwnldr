...
Protected Class JSONData
	#tag Method, Flags = &h21
		Private Sub AdvancePastWhiteSpace(inMB As MemoryBlock, ByRef pos As Integer)
		  ...
		  do
		    if pos > lastPos then
		      raise new JSONException( "A parsing error occurred at", 2, pos )
		      return
		    end if
	...
	#tag Method, Flags = &h0
		Function Child(index As Integer) As JSONData
		  dim r as JSONData = self.Value( index )
		  return r
	...
	#tag Method, Flags = &h0
		Sub Child(index As Integer, Assigns obj As JSONData)
	...
	#tag Method, Flags = &h0
		Function Child(name As String) As JSONData
		  dim r as JSONData = self.Value( name )
		  return r
	...
	#tag Method, Flags = &h0
		Sub Child(name As String, Assigns obj As JSONData)
		  self.Value( name ) = obj
	...
	#tag Method, Flags = &h21
		Private Function DecodeString(inMB As MemoryBlock, ByRef pos As Integer, current As JSONData, outMB As MemoryBlock) As String
		  dim lastPos as integer = inMB.Size - 1
		  if pos > lastPos then
		  ...
		  do
		    if pos > lastPos then
		      dim msg as string
		      if current.IsArray Then msg= "Missing , or ]" Else msg= "Missing , or }"
	...
	#tag Method, Flags = &h21
		Private Function DecodeValue(inMB As MemoryBlock, ByRef pos As Integer, current As JSONData, outMB As MemoryBlock) As Variant
	...
	#tag Method, Flags = &h21
		Private Sub EncodeString(s As String, settings As JSONData, outMBs() As MemoryBlock, ByRef outIndex As Integer, inMB As MemoryBlock)
	...
	#tag Method, Flags = &h21
		Private Sub EncodeValue(value As Variant, settings As JSONData, level As Integer, outMBs() As MemoryBlock, ByRef outIndex As Integer, inMB As MemoryBlock)
		  if value IsA JSONData then
		    JSONData( value ).Serialize( outMBs, outIndex, settings, level, inMB )
		    
		  elseif value IsA Dictionary then
		    dim child as JSONData = Dictionary( value )
		  ...
		  elseif value.Type = Variant.TypeText then
		    ...
		    if value.Type = Variant.TypeDouble or value.Type = Variant.TypeSingle or _
		      ...
		      elseif s.InStr( "nan" ) <> 0 then
		        ...
		        // 20190114;lbmonsalve;change format by str, osx error double values
	...
	#tag Method, Flags = &h21
		Private Function EnsureArray() As Boolean
		  if HasSetType <> kHasSetObject then
		    ...
		    raise new JSONException( "This JSONData is an obect", 13 )
		    return false
	...
	#tag Method, Flags = &h21
		Private Function EnsureObject() As Boolean
		  if HasSetType <> kHasSetArray then
		    ...
		    'raise new JSONException( "This JSONData is an array", 13 )
		    return false
	...
	#tag Method, Flags = &h21
		Private Function KeyToName(name As String) As String
		  dim r as string = DecodeHex( name )
		  r = r.DefineEncoding( Encodings.UTF8 )
		  return r
		End Function
	#tag EndMethod
		...
		Sub Load(JSONString As String)
		  ...
		  if JSONString = "" then
		    ...
		    if proceed then
		      ...
		      pos = pos + 1
		      dim stack() as JSONData
		      dim current as JSONData = self
	...
	#tag Method, Flags = &h21
		Private Sub LoadArray(inMB As MemoryBlock, ByRef pos As Integer, ByRef current As JSONData, stack() As JSONData, outMB As MemoryBlock)
		  dim inPtr As Ptr = inMB
	...
	#tag Method, Flags = &h21
		Private Sub LoadObject(inMB As MemoryBlock, ByRef pos As Integer, ByRef current As JSONData, stack() As JSONData, outMB As MemoryBlock)
		  dim inPtr As Ptr = inMB
	...
	#tag Method, Flags = &h21
		Private Function NameToKey(name As String) As String
		  dim r as string = name.ConvertEncoding( Encodings.UTF8 )
		  r = EncodeHex( r )
		  return r
	...
	#tag Method, Flags = &h1
		Protected Sub Serialize(outMBs() As MemoryBlock, ByRef outIndex As Integer, settings As JSONData, level As Integer, inMB As MemoryBlock)
		  dim outMB as MemoryBlock = outMBs( outMBs.Ubound )
	...
	#tag Method, Flags = &h21
		Private Sub Validate(value As Variant)
		  dim isGood as boolean = true // Assume it is
		  
		  select case value.Type
		  case Variant.TypeNil, Variant.TypeBoolean
		    
		  case Variant.TypeString, Variant.TypeCString, Variant.TypePString
		  ...
		  case Variant.TypeInteger
		    ...
		    // warning Variant.TypeLong -> Variant.TypeInt64
		    #if RBVersion< 2015.031
	...
	#tag Constant, Name = kVersion, Type = String, Dynamic = False, Default = \"0.0.180124", Scope = Private, Attributes = \"Hidden"
	#tag EndConstant
